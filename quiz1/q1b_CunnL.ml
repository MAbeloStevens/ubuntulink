(* Quiz 1 
   ...
   Student name 1: Lucas Cunnane 
   Student name 2: Alex Coroian 
*)
...
*)
let rec outgoing_nodes w (n:int) =
  match w with 
  | []->[] 
  | (node,child)::t -> 
    if node = n then child 
