...
let rec outgoing_nodes t (n:int) =
    ...
    | head::tail -> match head with
                    | (x,y) -> if x = n then y :: outgoing_nodes tail n else outgoing_nodes tail n


(** [nodes t] returns the list of nodes of the tree without duplicates. The order of the nodes in the list is irrelevant.
