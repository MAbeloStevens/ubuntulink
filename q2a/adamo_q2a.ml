...
let rec levels t =
   let rec levels_helper t i =
      match i with
      | 0 -> []
      | _ ->  (levels_helper t (i-1)) @ [level t i] 
   in
   let tHeight = height t in 
   levels_helper t (tHeight - 1)
...
let rec paths_to_leaves t =
  match t with
  | Empty -> [[]]
  | Node(x, l, r) -> (List.map (fun l -> 0 :: l) (paths_to_leaves l)) @ (List.map (fun l -> 1 :: l) (paths_to_leaves r))
