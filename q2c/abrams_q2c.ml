...
let rec level t i =
   ...
   | Empty -> []
   | Node(d,Empty,Empty) -> []
   | Node(d,lt,Empty) -> (level lt (i-1))
   | Node(d,Empty,rt) -> (level rt (i-1)) 
   | Node(d,lt,rt) -> (level lt (i-1)) @ (level rt (i-1))
...
let rec levelsCount t i =

   if ((level t i) = []) then (
      []
   ) else (
      [(level t i)] @ (levelsCount t (i+1))
   )
...
let rec paths_to_leaves t =
  ...
  | Empty ->  []
  | Node(d,Empty,Empty) ->  [[]]
  | Node(d,lt,rt) ->    let left_paths = List.map(fun p -> 0 :: p) (paths_to_leaves lt) in
