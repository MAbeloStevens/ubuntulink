...
let rec level_helper t i l = 
  match t with
  | Empty -> []
  | Node(d, lt, rt) -> if (l=i) then [d] else ((level_helper lt i (l+1)) @ (level_helper rt i (l+1)) )
...
*) 
let rec levels_maker t i=
  match t with
  | Empty -> [] 
  | Node(d, lt, rt) -> if (List.length (level t i) = 0) then [] else  (level t i)::(levels_maker t (i+1)) 
      
let rec levels t =
  List.append [] (levels_maker t 1)
...
*)
let rec pbt h d =
  if (not (h = 0)) then Node(d, pbt (h-1) d, pbt (h-1) d) else Empty
